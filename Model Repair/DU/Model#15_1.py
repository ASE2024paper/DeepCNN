...
model.summary()
result=model.fit(train_image,train_label, epochs=epochs, batch_size=32, validation_data=(val_image,val_label))
last_epoch_loss = result.history['loss'][-1]
last_epoch_acc =  result.history['accuracy'][-1]
print("Final Result: loss={0} : accuracy={1} : fileName: {2}".format(last_epoch_loss, last_epoch_acc, os.path.basename(__file__)))